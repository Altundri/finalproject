...
if(!empty($_SESSION['admin'])){
	...
	if(!empty($_GET['barang'])){
		...
		$data[] = $tgl;
		$data[] = $id;
		$sql = 'UPDATE barang SET id_kategori=?, nama_barang=?,deskripsi=?,warna=?, 
	...
	if(!empty($_GET['cari_barang'])){
		...
		if($cari == '')
		{
			...
			$sql = "select barang.*, kategori.id_kategori, kategori.nama_kategori
					...
					where barang.id_barang like '%$cari%' or barang.nama_barang like '%$cari%' or barang.warna like '%$cari%'";
			$row = $config -> prepare($sql);
			$row -> execute();
			$hasil1= $row -> fetchAll();
	?>
